...
import { Link, Outlet, useLoaderData, useFetcher, useLocation, useParams, useOutletContext} from "@remix-run/react";
...
export const action = async (args: ActionArgs) => {
    ...
    switch (form._action) {
            ...
            if(form.projectId) {     
                if (form.count == 0) {
                    ...
                    if (form.type === 1) {
                        ...
                        useService("mail", { user, locale }).sendFinallyMail(parseInt(id, 10), email);
                    }
                }
        ...
        case 'completeChange' : {
            const user = await isAuthenticated(args)
            const { email } = await useService('designer').getUserEamilByDemandId(parseInt(id, 10))
            useService("mail", { user, locale}).sendComplateMail(parseInt(id, 10), email, form.status, locale);
        ...
        case 'addEmpSource': {      
            ...
            if (id) {
                return createEmpAttachments(parseInt(id, 10), result.data.name, result.data.link)
            }
        }
        case 'editEmpSource': {
            const result = await DemandEditAttachmentValidator.safeParse(form)
...
async function createEmpAttachments(id: number, name: string, link: string) {
    const service = useService("demand");
    await service.createEmpAttachments(name,link, id)
    return json({ code: ResultCode.OK })
}
...
export default function Page () {
    ...
    const handleAddEmpSource = (name: string, link: string) => {
        mutation.submit({ _action: "addEmpSource", name, link, id: 0}, { method: "post", encType: "application/json" })
    }
    const handleDelEmpSource = (id: number) => {
        mutation.submit({ _action: "delEmpsSource", id}, { method: "post", encType: "application/json" })
    }
    const handleEditEmpSource = (id: number, name: string, link: string) => {
        mutation.submit({ _action: "editEmpSource", name, link, id}, { method: "post", encType: "application/json" })
    }
    return (
        <>
          {pathname.endsWith("d") &&  <DesignerDetail
            ...
            {pathname.endsWith("g") && <EmployerDetail
                ...
                onAddSource={handleAddEmpSource}
                onDelSource={handleDelEmpSource}
                onEditSource={handleEditEmpSource}
