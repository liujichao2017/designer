import {
  Link, useParams, useLocation, Outlet, useSearchParams,
  useOutletContext, useFetcher, useLoaderData, useNavigate
...
export async function loader (args: LoaderArgs) {
  ...
  // const emailList = await useService('userMessage').getLatestMessageByProject()
  // console.log('messageListRes', messageListRes)
  return json({
    user, projectId: id, messageData: messageListRes.data,
    messageTotal: messageListRes.total, code: ResultCode.OK, link: process.env.SHARE_URL
  })
}
    ...
    case "submitMessage":
      {
        ...
        // }
        // let locale = await i18next.getLocale(args.request)
        // useService("mail", { user, locale }).sendEmailToUnreadMessageClientOrDesigner()
        // return json({})
...
export default function Page () {
    ...
    mutation.submit(
      {
        ...
        imageFileList: JSON.stringify(imageFileList), userId: user.id, userRole: 'backAdmin', projectId: projectId,
      },
  ...
  useEffect(() => {
    if (mutation.state === "idle" && mutation.data && mutation.data?.code === ResultCode.OK && mutation.data.msg == 'uploadImageSuccess') {
      ...
      temp.push({
        fileType: imageInfo.fileType, fileThumbnailUrl: imageInfo.fileThumbnailUrl,
    ...
    if (mutation.state === "idle" && mutation.data && mutation.data?.code === ResultCode.OK && mutation.data.msg == 'uploadFileSuccess') {
      ...
      let temp = [...imageFileList]
      temp.push({
        fileType: fileInfo.fileType, fileThumbnailUrl: fileInfo.fileThumbnailUrl,
  ...
  return (<div className="pb-[6.75rem]">
    ...
    <div className="bg-white rounded-lg h-[60vh] overflow-y-auto" id="messgeContainer">
