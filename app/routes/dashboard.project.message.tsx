...
import { handle } from "~/root";
  ...
  export const loader = async (args: LoaderArgs) => {
    ...
    const messageListRes = await useService('userMessage').getMessageListByProjectId(demandDetail.id, parseInt(pageIndex))
    return json({ user, demandDetail, messageData: messageListRes.data, messageTotal: messageListRes.total, code: ResultCode.OK,  link: process.env.SHARE_URL })
  }
  ...
  export const action = async (args: ActionArgs) => {
    ...
    switch (_action) {
      ...
      case "submitMessage":
      {
        ...
        //   return fault(ResultCode.FORM_INVALID)
        // }
        return json(await service.saveMessage(form.messageContent, form.userId, 
  ...
  export default () => {
    const { demandDetail, user, messageData, messageTotal, } = useLoaderData<typeof loader>()
    ...
    useEffect(() => {
      if(messageData.length > 0 ){
        ...
        // totalNum.current = totalNum.current + messageData.length
    ...
    const msgList = [
      {
        "userAvatar" : "~/images/default-client-avatar.png",
        "content" : "除了查看给出的参考网站样式，也可以参考我附件上的样式。",
        "userType" : "1",
        "userName" : "名字名字名字",
        "sendTime" : "2天前",
        "fileList" : [
      ...
      {
        "userAvatar" : "~/images/default-client-avatar.png",
        "content" : "設計風格參考網站：http://www.zhanku.com",
        "userType" : "1",
        "userName" : "名字名字名字",
        "sendTime" : "3天前",
      },
      ...
      {
        "userAvatar" : "~/images/default-client-avatar.png",
        "content" : "文字文字文字文字文字文字文字文字文字文字文字文字文字文字文字文字文字文字。",
        "userType" : "2",
        "userName" : "我是設計呀～",
        "sendTime" : "6天前",
        "fileList" : [
    ...
    const sendMessage = () => {
      ...
      if(messageContent.trim().length < 10){
        toast({
          title: t('prompt'),
          description: t('messagePromptDescription')
        })
        return
      }
      ...
      mutation.submit(
        ...
        imageFileList: JSON.stringify(imageFileList), userId: user.id, projectId: demandDetail.id,
        },
    ...
    const toNextPage = () => {
      ...
      navigate("/dashboard/project/message?id=" + demandDetail.id + "&pageIndex=" + (currentPageIndex.current) )
    }
    ...
    useEffect(() => {
      ...
      if (mutation.state === "idle" && mutation.data?.code === ResultCode.OK && mutation.data.msg == 'saveMessageSuccess') {
        ...
        setMessageList([])
        navigate("/dashboard/project/message?id=" + demandDetail.id )
      }
    ...
    return (<div className="pb-[6.75rem]">
      ...
      <div className="bg-white rounded-lg p-5 h-[60vh] overflow-y-auto" id="messgeContainer">
        {/* <div className="flex w-full h-[50vh] justify-center items-center">
            ...
            <div className="w-12">
              <img src={DefaultClientAvatar} className="w-12 h-12 rounded-full"></img>
            </div>
            {/* leave message start */}
            <div className="flex flex-col ml-5 w-full">
              <textarea className="textarea textarea-bordered textarea-xl w-full h-32" 
              value={messageContent} placeholder={t('messageTip')}
              ...
              <div className="flex justify-between mt-5">
                  ...
                  <div className="cursor-pointer w-24 h-7 border flex justify-center rounded mb-2.5">
                    <span className="text-[#868686] text-sm leading-7" onClick={() => {
                      ...
                      if(imageFileList.length >=5){
                        toast({
                          title: t('prompt'),
                          description: t('messageUploadFileCountLimit')
                        })
                        return
                      }
                ...
                <div className="cursor-pointer bg-[#2F4CDD] w-24 h-7 flex justify-center rounded" onClick={sendMessage}>
                  <span className="text-white text-sm leading-7">{t('leaveMessage')} </span>
                </div>
              </div>
            </div>
            {/* leave message end  */}
            ...
            messageList.map((item ,index) => {
              return (
                <div key={index} className="flex align-start mt-10">
                  <div className="w-12">
                    <img src={item.user_avatar } className="w-12 h-12 rounded-full"></img>
                  </div>
                  <div className="flex flex-col ml-5 w-full text-sm">
                    <div className="flex">
                      ...
                      <div className="bg-[#D2D8FC] rounded h-5 ml-2 flex justify-center">
                        <span className="text-[#2A387E] text-xs leading-5 px-2">{ item.user_role == 'client' ? t('client') : t('designer') }</span>
                      </div>
          ...
          <UploaderDialog upload={upload} allowedTypes={["image/png", "image/jpeg","application/pdf", 
            ...
            { totalPageNum.current == (currentPageIndex.current + 1) ? (
              <div className="font-bold text-sm mt-5 text-center">{t('messageAllLoaded')}</div>
            ) : ''}
